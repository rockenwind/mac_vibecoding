...
- 최근 스캔 이력 저장과 스캔 결과 2개 직접 비교
- 최근 스캔 이력에서 과거 결과 다시 열기
- 최근 스캔 이력 수동 삭제
- 새 발견, 해결됨, 유지됨, 오탐 처리 항목 분리
- 스캔 규칙의 구현 출처, 탐지 방식, 영향도, 필요 조치 표시
- 규칙별 사용 여부 설정
...
flowchart LR
  ...
  History --> Compare["스캔 2개 직접 비교"]
  History --> Suppression["오탐 설정"]
  Scanner --> RuleDetails["규칙 출처와 분석 기준"]
```
...
sequenceDiagram
  ...
  S-->>A: 발견 항목과 심각도 요약
  A->>H: 스캔 결과 저장
  H-->>A: 같은 저장소의 직전 스캔 비교
  A-->>W: 스캔 결과, 비교 결과, 규칙 상세, 오탐 분리 결과
...
| 비교 | 완료 | 저장된 스캔 결과 2개를 직접 선택해 새 발견, 해결됨, 유지됨 항목을 비교합니다. |
| 규칙 출처 | 완료 | 각 스캔 규칙의 구현 파일, 탐지 방식, 영향도, 필요 조치, 한계를 표시합니다. |
...
| `GET /api/scans/settings` | 오탐, 규칙 설정, 규칙 상세 메타데이터 조회 |
| `PATCH /api/scans/settings` | 오탐 처리, 규칙 사용 여부 변경 |
...
flowchart TD
  ...
  HistoryJson --> DirectCompare["스캔 2개 직접 비교"]
  HistoryPostgres --> DirectCompare
...
6. 최근 스캔 이력에서 스캔 A와 스캔 B를 선택해 새 발견, 해결됨, 유지 중 항목을 직접 비교합니다.
7. 최근 스캔 이력을 클릭해 과거 결과를 다시 확인합니다.
8. 더 이상 필요 없는 최근 스캔 이력은 삭제 버튼으로 정리합니다.
9. 스캔 규칙과 분석 기준 영역에서 규칙의 구현 출처, 탐지 방식, 영향도, 필요 조치를 확인합니다.
10. 오탐 처리와 규칙 사용 여부를 조정합니다.
11. 필요하면 JSON 보고서를 열거나 Markdown 보고서와 보안 체크리스트를 내려받아 상세 결과를 확인합니다.
12. GitHub App installation ID가 있으면 GitHub Issue를 생성합니다.
13. 예약 스캔을 저장하고 실행 시간이 지난 예약을 수동으로 실행해 새 취약점과 해결된 취약점 알림 후보를 확인합니다.
14. 운영 환경에서 변경 작업을 수행할 때 관리자 토큰을 입력합니다.

## 관리자 토큰

운영 환경에서 수동 스캔 실행, 예약 저장/삭제, 규칙 설정, 오탐 처리, 스캔 이력 삭제, GitHub Issue 생성 같은 변경 작업은 `SCAN_ADMIN_TOKEN`으로 보호할 수 있습니다.

Render 웹 서비스 환경 변수에 충분히 긴 무작위 값을 추가합니다.

```text
SCAN_ADMIN_TOKEN=충분히-긴-무작위-관리자-토큰
```

값이 설정되어 있으면 변경 API는 `Authorization: Bearer <토큰>` 헤더가 없는 요청을 `401`로 거부합니다. 웹 화면의 `관리자 토큰 / Admin token` 입력란에 같은 값을 넣으면 브라우저가 변경 요청에만 해당 헤더를 붙입니다. 읽기 전용 이력 조회와 보고서 다운로드는 기존처럼 토큰 없이 동작합니다.

## 예약 스캔과 변경 알림

예약 스캔은 Repository scan 서비스 안에서 저장소별 점검 주기를 관리합니다. 현재 단계에서는 화면과 API에서 예약을 저장하고, `run-due` API를 호출해 실행 시간이 지난 저장소를 점검합니다.

예약 실행 결과는 기존 스캔 이력에 저장되며, 같은 저장소의 이전 스캔과 비교해 다음 내용을 보여줍니다.
...
이 값이 있으면 JSON 파일 대신 데이터베이스에 스캔 이력과 설정을 저장합니다. 저장되는 설정에는 오탐 처리, 규칙 사용 여부, 예약 스캔 정보가 포함됩니다.
